...
{
    struct MainPoolRegion* region = &gMainPool.regions[region_idx];
    sprintf(__n64Assert_MessageBuf, "%salloc failed for %d bytes in region %d\n%x-%x", freeable ? "freeable " : "", size, region_idx, region->start, region->end);
    __n64Assert((char*) file, line, __n64Assert_MessageBuf);
}

#if MAIN_POOL_REGIONS_COUNT > 1
void *main_pool_alloc_ex(int region, u32 size, u32 alignment) {
    if (region != MAIN_POOL_REGION_MAIN)
    {
...
#else
void *main_pool_alloc_ex(UNUSED int region, u32 size, u32 alignment) {
    return main_pool_region_alloc_from_start(MAIN_POOL_REGION_MAIN, size, alignment, MAIN_POOL_ALLOC_FORCE);
#endif
}
...
void main_pool_free(void *addr) {
    ...
    aggressf(header->magic == MAIN_POOL_FREEABLE_HEADER_MAGIC_RIGHT, "Incorrect magic for free");
